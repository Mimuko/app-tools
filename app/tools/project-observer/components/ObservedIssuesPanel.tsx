...
export function ObservedIssuesPanel({ issues }: ObservedIssuesPanelProps) {
  return (
    <Panel title="観測された課題" hint="未対応の新規課題は要注目に含めません">
      {issues.length === 0 ? (
        <p className="text-base text-slate-500">要約すべき課題シグナルはありません。</p>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="font-mono text-sm text-cyan-600/80">{issue.issueKey}</span>
                  <span className="text-sm text-slate-500">{st.label}</span>
                </div>
                <p className="mt-2 text-base font-medium text-slate-100">{issue.title}</p>
                <p className="mt-1 text-sm text-slate-500">{issue.reasons.join(' · ')}</p>
                ...
                <div className="mt-2 flex gap-3 text-sm uppercase tracking-wider text-slate-600">

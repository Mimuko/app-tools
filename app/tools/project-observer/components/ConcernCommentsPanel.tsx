...
const SIGNAL_STYLES: Record<ConcernComment['signal'], string> = {
  ...
  pending_reply: 'text-cyan-700 border-cyan-500/35 dark:text-cyan-400 dark:border-cyan-500/30',
};
...
export function ConcernCommentsPanel({ comments }: ConcernCommentsPanelProps) {
  return (
    <Panel title="懸念のあるコメント" hint="Backlog上の発言から拾ったシグナル">
      {comments.length === 0 ? (
        <p className="text-base obs-text-muted">直近の懸念コメントはありません。</p>
      ) : (
        <ul className="space-y-4">
          {comments.map((comment) => (
            ...
            >
              <blockquote className="obs-divider border-l-2 pl-3 text-base leading-relaxed obs-text-secondary">
                「{comment.excerpt}」
              </blockquote>

    ...
    <section className="mb-8 grid gap-4 lg:grid-cols-[1fr_1.2fr]">
      ...
      <div className="rounded-lg border border-cyan-900/25 bg-[#0a1018]/90 px-5 py-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-base text-slate-300">{status.label}</span>
          <span className="text-slate-700">|</span>
          <span className="text-sm text-slate-500">次アクション</span>
          <span className={`text-base font-medium ${nextAction.className}`}>
            {nextAction.label}
          </span>
        </div>
        <p className="mt-3 text-base leading-relaxed text-slate-400">{sharingObservation}</p>
      </div>
    </section>
  );
}

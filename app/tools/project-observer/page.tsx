...
export const metadata = {
  title: 'Project Observer | 案件観測',
  description: '状況共有・認識同期・負荷可視化の観測ダッシュボード',
};
...
export default async function ProjectObserverPage() {
  ...
  return (
    <ObserverShell
      ...
      subtitle={
        live
          ? 'Backlog実データ — ディレクターチーム向け観測'
          : 'モックデータ — ディレクターチーム向け観測'
      }
      backHref="/tools"
      backLabel="ツール一覧"
    >
      ...
      {live && (
        <p className="mb-4 rounded border border-emerald-900/40 bg-emerald-950/20 px-3 py-2 text-xs text-emerald-300/90">
      ...
      <footer className="mt-10 border-t border-cyan-900/20 pt-6 text-center">
        ...
        <Link
          href="/tools"
          className="mt-2 inline-block text-xs text-cyan-600/70 hover:text-cyan-500"
        >
          ツール一覧へ戻る
        </Link>
      </footer>

...
export function ContextNotesPanel({ notes }: ContextNotesPanelProps) {
  return (
    <Panel title="補助情報" hint="停滞の件数ではなく、なぜ・共有・認識の観点">
      {notes.length === 0 ? (
        <p className="text-sm text-slate-500">補足すべき文脈はありません。</p>
      ) : (
        <ul className="space-y-3">
          {notes.map((note) => (
            ...
            >
              ...
              <p className="mt-1 text-sm font-medium text-slate-200">{note.title}</p>
              <p className="mt-1 text-xs leading-relaxed text-slate-500">{note.detail}</p>
            </li>
          ))}
        </ul>
      )}

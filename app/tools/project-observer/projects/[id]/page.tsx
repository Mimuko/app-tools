...
import { getAllProjectIds, getProjectDetail } from '../../mock/projects';
...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  return (
    ...
    >
      ...
      {isUsingLiveBacklog() && (
        <p className="mb-4 text-xs text-emerald-400/80">
      ...
      <div className="mb-8 rounded-lg border border-cyan-900/30 bg-gradient-to-r from-cyan-950/30 to-transparent px-5 py-4">
        <p className="font-mono text-[10px] uppercase tracking-widest text-cyan-600/70">
          観測メモ
        </p>
        <p className="mt-2 text-sm leading-relaxed text-slate-300">{project.observerNote}</p>
      ...
      <footer className="mt-10 text-center">
        <Link
          ...
          className="font-mono text-xs uppercase tracking-widest text-cyan-600/70 hover:text-cyan-500"
        >
          ← 観測一覧に戻る
        </Link>
      </footer>
...
function StatPill({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="rounded-md border border-cyan-900/25 bg-[#0a1018]/80 px-3 py-2 text-center">
      <p
        className={`font-mono text-lg tabular-nums ${emphasized ? 'text-cyan-300/90' : 'text-slate-600'}`}
      >
        {value}
      </p>
